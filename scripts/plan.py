...
[G] 海報提示詞撰寫（poster_vars.json 的 narrative 欄位，dayN_prompt.md 主體）
  ...
  - ★主角/公路車「外型」走圖生圖（img2img），實際產圖以 主角.png 參考圖為準。
    模板（prompt.md.j2 / cover_prompt.md.j2）開頭已固定帶一句**原話保留**的生成指示：
    「請依照我上傳的照片生成角色 外觀保留真實臉型、五官比例、髮型輪廓 風格轉換為：
    3D Q版公仔（高級模型公仔 / 微縮場景 diorama） 特徵：頭大身小比例、臉部仍可辨識
    為本人、玻璃珠眼睛、細膩皮膚質感、表情自然、有生命力。」＋公路車縮小模型細節。
    故 narrative **不必也不要再描述主角/公路車外型或重述風格樣板**，也不引用已移除
    的「主角.md」；narrative 專注當日旅程故事/主視覺場景/動作/小分身/地理方位。
